...
# necessário ser root
if [ "$(id -u)" != "0" ]; then
   setup_log "Desculpe! Este script deve ser executado como root." 1>&2
   exit 1
fi
...
setup_log "Criando link simbólico para a pasta de aplicações..."
ln -s /var/$VENDOR_NAME/apps /home/$DEPLOYER_USERNAME/apps

setup_log "Criando link simbólico para a pasta storage..."
ln -s /var/$VENDOR_NAME/storage /home/$DEPLOYER_USERNAME/storage

setup_log "Criando link simbólico para a pasta de backups..."
